...
public class Auto {
    ...
    public void quitarRueda(Rueda r){
        if (ruedas.size()>0) {
            if (ruedas.contains(r)) {
                 ruedas.remove(r);
                 ruedas.remove(posRueda);
                 System.out.println("Se retiro la rueda");
            }else 
                System.out.println("No se puede quitar la rueda...");
        }else 
            System.out.println("NO tenemos mas ruedas");
    }
